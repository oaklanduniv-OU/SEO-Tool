...
reports_dir = Path(__file__).resolve().parent.parent / "static" / "reports" / "linkchecker"
reports_dir.mkdir(parents=True, exist_ok=True)

# Define paths
main_report_path = reports_dir / "linkchecker-report.txt"
timestamp = datetime.datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
backup_path = reports_dir / f"linkchecker-report-BACKUP-{timestamp}.txt"
...
def run_linkchecker(url):
    ...
    try:
        ...
        if timestamped_output_path.exists():
            ...
            timestamped_output_path.unlink()  # ✅ Delete the intermediate timestamped file
...
if __name__ == "__main__":
    test_url = "https://www.oakland.edu/english"
    run_linkchecker(test_url)
    cleanup_old_backups(reports_dir)
