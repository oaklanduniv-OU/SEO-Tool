...
groupedArray.sort((a, b) => {
  const brokenCountA = a[1].filter(link => link.status === "broken").length;
  const brokenCountB = b[1].filter(link => link.status === "broken").length;
  return brokenCountB - brokenCountA;
});      break;
  }
