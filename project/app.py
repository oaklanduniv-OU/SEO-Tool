...
def run_script():
    action = request.form.get('action')  # Get the action from the button clicked

    # Get the section or URL depending on the action
    section = request.form.get('section')
    url = request.form.get('url')  # Get the URL for the single-page scan

    # Define a dictionary mapping actions to the corresponding function calls
    if action == 'sitewide_report':
        try:
            crawlSitemap()  # Adjust if needed for your use case
            return jsonify({"message": "Sitewide report generated successfully!"})
        except Exception as e:
            return jsonify({"message": f"Error executing Sitewide report: {e}"}), 500
    elif action == 'section_report':
        try:
            if section:
                crawlSection("/" + section)  # Use the selected section from the dropdown
                return jsonify({"message": f"Section report for {section} generated successfully!"})
            else:
                return jsonify({"message": "No section selected"}), 400
        except Exception as e:
            return jsonify({"message": f"Error executing Section report: {e}"}), 500
    elif action == 'single_page_report':
        try:
            if url:
                crawlSinglePage(url)  # Use the URL provided by the user
                return jsonify({"message": f"Single page report for {url} generated successfully!"})
            else:
                return jsonify({"message": "No URL provided for the single-page scan"}), 400
        except Exception as e:
            return jsonify({"message": f"Error executing Single Page report: {e}"}), 500
    else:
